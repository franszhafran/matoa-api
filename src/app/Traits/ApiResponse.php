...
trait ApiResponse
{
    ...
    public function handleException(Exception $e): JsonResponse
    {
        ...
        if ($e instanceof ValidationException) {             
            if(is_string($e->validator)) return $this->sendError($e->validator, 'Error', Response::HTTP_UNPROCESSABLE_ENTITY);
            $errors = $e->validator->errors()->getMessages();

            return $this->sendError($errors, 'Error', Response::HTTP_UNPROCESSABLE_ENTITY);
        }

        if($e instanceof AuthenticationException) return $this->sendError($e->getMessage(), 'Error', 401);
